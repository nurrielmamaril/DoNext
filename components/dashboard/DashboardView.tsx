    ...
    <div className="relative flex h-full flex-col overflow-hidden md:block">
      ...
      <div className="pointer-events-auto flex items-center justify-between pb-4 md:pb-6">
        ...
        <div className="flex gap-2">
          ...
          {/* No New task button here on purpose: this dialog has no client
              picker, so a task created from the dashboard would land in Inbox
              with no way to choose where it goes. Add tasks from inside a
              client's own list instead — same on phone and desktop. */}
        </div>
      </div>
      ...
      {!isLoading && (
        <div className="pointer-events-auto grid gap-3 sm:grid-cols-3 md:gap-4">
          <DashboardWidget
            icon={AlertTriangle}
            title="Overdue"
